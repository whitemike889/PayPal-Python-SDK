...
class TestWebProfile(unittest.TestCase):
    ...
    def test_create(self, mock):
        ...
        mock.assert_called_once_with(self.web_profile.api,'/v1/payment-experience/web-profiles',self.web_profile_attributes, {'PayPal-Request-Id' : ANY}, None)
    ...
    def test_find(self, mock):
        ...
        mock.assert_called_once_with(web_profile.api, '/v1/payment-experience/web-profiles/'+self.profile_id)

    @patch('test_helper.paypal.Api.patch', autospec=True)
    def test_replace(self, mock):
        self.web_profile.id = self.profile_id
        response = self.web_profile.replace(self.web_profile_replace_attributes)

        mock.assert_called_once_with(self.web_profile.api, '/v1/payment-experience/web-profiles/' + self.profile_id
            , self.web_profile_replace_attributes, {'PayPal-Request-Id': ANY}, None)
        self.assertEqual(response, True)
